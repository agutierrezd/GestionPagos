...
{
	$value = "";
				if($table=="hoja_ruta_2020_files" && $field=="hr_file")
	{
		;
		return $value;
	}
				if($table=="hoja_ruta_ob" && $field=="ob_anexo")
	{
		;
		return $value;
	}
				if($table=="hoja_ruta_op" && $field=="op_anexo")
	{
		;
		return $value;
	}
...
function GetLWWhere($field, $ptype, $table = "")
{
	...
	if(!$table)
		$table = $strTableName;
		if($table=="hoja_ruta_2020_pn" && $field=="sys_dep")
	{
		$strWhere = " id= 23 ";
		return $strWhere;
	}
...
function GetDefaultValue($field, $ptype, $table="")
{
				...
				if($table=="global_users" && $field=="groupid")
	{
		return 1;
				...
				if($table=="hoja_ruta_2020_pn" && $field=="hr_concepto_id")
	{
		return 8;
	}
				if($table=="hoja_ruta_2020_pn" && $field=="hr_fechaingreso")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
	}
				if($table=="hoja_ruta_2020_pn" && $field=="not_date")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
	}
				if($table=="hoja_ruta_2020_pn" && $field=="evento_type_id_fk")
	{
		return 14;
				...
				if($table=="hoja_ruta_2020_pn" && $field=="user_from")
	{
		return $_SESSION["UserID"];
	}
				if($table=="hoja_ruta_2020_pn" && $field=="last_act_user")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
	}
				if($table=="hoja_ruta_2020_files" && $field=="sys_user")
	{
		return $_SESSION["UserID"];
	}
				if($table=="hoja_ruta_2020_pn1" && $field=="hr_concepto_id")
	{
		return 8;
	}
				if($table=="hoja_ruta_2020_pn1" && $field=="hr_fechaingreso")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
	}
				if($table=="hoja_ruta_2020_pn1" && $field=="not_date")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
	}
				if($table=="hoja_ruta_2020_pn1" && $field=="last_act_user")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
	}
				if($table=="hoja_ruta_ob" && $field=="ob_user")
	{
		return $_SESSION["UserID"];
	}
				if($table=="hoja_ruta_ob" && $field=="ob_fechauser")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
	}
				if($table=="hoja_ruta_op" && $field=="op_user")
	{
		return $_SESSION["UserID"];
	}
				if($table=="hoja_ruta_op" && $field=="op_fechauser")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
	}
				if($table=="hoja_ruta_2020_pn2" && $field=="hr_concepto_id")
	{
		return 8;
	}
				if($table=="hoja_ruta_2020_pn2" && $field=="hr_fechaingreso")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
	}
				if($table=="hoja_ruta_2020_pn2" && $field=="not_date")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
	}
