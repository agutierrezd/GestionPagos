...
function checkTableName($shortTName, $type=false)
{
	...
	if ("admin_users" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("hoja_ruta_2020_pn" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("q_bandejaentrada_supervisores" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("hoja_ruta_2020_files" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("hoja_ruta_2020_tracking" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("dependencia" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("cargo" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("hoja_ruta_event_type" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("informe_intersup_anexos_tipo" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("nomina_cont_meses" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("contrato_estados" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("hoja_ruta_2020_pn1" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("q_info_cont" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("q_buscador_usuarios_all" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("hoja_ruta_ob" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("hoja_ruta_2020" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("hoja_ruta_op" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("contractor_master" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("ep_obligaciones" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("ep_op" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("ep_obligaciones_temp" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("ep_registros" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("ep_op_temp" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
...
function GetTablesList($pdfMode = false)
{
	...
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("hoja_ruta_2020_pn");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
		$arr[]="hoja_ruta_2020_pn";
	}
	$tableAvailable = true;
	if( $checkPermissions ) {
	...
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("hoja_ruta_2020_files");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
		$arr[]="hoja_ruta_2020_files";
	}
	$tableAvailable = true;
	if( $checkPermissions ) {
	...
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("hoja_ruta_event_type");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
		$arr[]="hoja_ruta_event_type";
	}
	$tableAvailable = true;
	if( $checkPermissions ) {
	...
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("hoja_ruta_2020_pn1");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
		$arr[]="hoja_ruta_2020_pn1";
	}
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("q_info_cont");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
	...
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("q_buscador_usuarios_all");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
		$arr[]="q_buscador_usuarios_all";
	}
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("hoja_ruta_ob");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
	...
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("hoja_ruta_2020");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
	...
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("hoja_ruta_op");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
	...
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("ep_op");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
	...
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("ep_obligaciones_temp");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
		$arr[]="ep_obligaciones_temp";
	}
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("ep_registros");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
	...
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("ep_op_temp");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
	...
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("hoja_ruta_2020_pn2");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
...
function ReadUserPermissions($userID = "")
{
	...
	if($userID != "Guest")
	{
		...
		$sql = "select ".$gConn->addFieldWrappers( "GroupID" )
			...
			." from ". $gConn->addTableWrappers( "gestionpagos_ver_103_ugmembers" )
	...
	$sql = "select ". $gConn->addFieldWrappers( "TableName" )
		...
		." from ". $gConn->addTableWrappers( "gestionpagos_ver_103_ugrights" )
...
function SetAuthSessionData($pUsername, &$data, $password, &$pageObject = null, $fireEvents = true )
{
	...
	$_SESSION["_hoja_ruta_2020_pn_OwnerID"] = $data["Username"];
